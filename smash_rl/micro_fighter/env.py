...
import numpy as np
from smash_rl_rust import MicroFighter
...
class MFEnv(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None, view_channels: Optional[Tuple[int, int, int]] = None):
        """
        Args:
        ...
        if self.render_mode == "human":
            pygame.init()
            self.screen = pygame.display.set_mode((IMG_SIZE * IMG_SCALE, IMG_SIZE * IMG_SCALE))
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict[str, Any]]:
        ...
        for hbox in hboxes:
            ...
            points = (points + self.game.get_screen_size() / 2) * (IMG_SIZE / self.game.get_screen_size())
            ...
            player_channel = player_channel * inv_box_arr + box_arr * float(hbox.is_player)
            state_channel = state_channel * inv_box_arr + box_arr * (int(hbox.char_state) / 8)
            box_channel = box_channel * inv_box_arr + box_arr
        
        self.channels[0] = hit_channel
        self.channels[1] = dmg_channel
        self.channels[2] = player_channel
        self.channels[3] = state_channel
        self.channels[4] = box_channel
        
        return np.stack(self.channels), 0.0, False, False, {}

    def reset(
        self, *args, seed=None, options=None
    ) -> tuple[np.ndarray, dict[str, Any]]:
        obs = np.zeros([IMG_SIZE, IMG_SIZE])
        return obs, {}
    ...
    def render(self):
        if self.render_mode == "human":
            ...
            pygame.transform.scale(view_surf, [IMG_SIZE * IMG_SCALE, IMG_SIZE * IMG_SCALE], self.screen)
            pygame.display.flip()
            self.clock.tick(60)
