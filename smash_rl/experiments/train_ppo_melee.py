...
train_steps = (
    256 * 16
)  # Number of steps to step through during sampling. Total # of samples is train_steps * num_envs.
...
eval_steps = 2  # Number of eval runs to perform.
...
env = SyncVectorEnv(
    [
        lambda: TimeLimit(
            MeleeEnv(num_frames=num_frames),
            time_limit,
        )
...
assert isinstance(env.envs[0].env, MeleeEnv)
...
for step in tqdm(range(iterations), position=0):
    ...
    if (step + 1) % eval_every == 0:
        eval_done = False
        with torch.no_grad():
            test_env = TimeLimit(MeleeEnv(num_frames=num_frames), max_eval_steps)
